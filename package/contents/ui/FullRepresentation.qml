	...
	ScrollView {
		...
		ListView {
			...
			delegate: RowLayout {
				...
				// 	source: {
				// 		if (issue.state == 'open') {
				// 			return plasmoid.file("", "icons/octicon-issue-open.svg")
				// 		} else { // 'closed'
				// 			return plasmoid.file("", "icons/octicon-issue-closed.svg")
				// 		}
				// 	}
				AppletIcon {
					source: {
					...
					// Rectangle { color: "transparent"; border.width: 1; border.color: "#f00"; anchors.fill: parent}
				}
				...
				ColumnLayout {
					...
					PlasmaComponents.Label {
						...
						// text: '#19 opened 7 days ago by RustyRaptor'
						wrapMode: Text.Wrap
						font.family: 'Helvetica'
						font.pointSize: -1
